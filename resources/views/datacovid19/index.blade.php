...
@foreach($data as $d)
  ...
  point_pasien_id_{{ $d->id }}.on('click', function (e) { 
    ...
    pop.setContent("Nama : {{$d->nama}}<br>Jenis : {{$d->jenis}}<br>No. KTP : {{$d->ktp}}<br>Alamat : {{$d->alamat}}<br>Keluhan : {{$d->keluhan_sakit}}<br>Riwayat Perjalanan : {{$d->riwayat_perjalanan}}<br>");
    map.openPopup(pop);
  });
@endforeach
